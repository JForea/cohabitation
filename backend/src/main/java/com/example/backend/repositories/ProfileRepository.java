...
public interface ProfileRepository extends ListCrudRepository<Profile, Long> {
    ...
    List<Profile> findAllByApartment_IdAndUserNotAndLeftAtNull(Integer apartmentId, User user);
    List<Profile> findAllByApartment_IdAndIdNot(Integer apartmentId, Long profileId);
    List<Profile> findAllByApartment_IdAndLeftAtNull(Integer apartmentId);
    @Modifying
    @Query("""
        UPDATE Profile p
        SET p.leftAt = :leftAt
